...
output_spreadsheetId = '1UOlnRrvdojCzZvaJ3dk91uO-TwF5YzpSN5-mEO3Zvu0'
output_data_range = 'Results'

raw_data_dict = {
        'spreadsheetId': {
                'TW': '11MgFt9HS6pesyR7uI-TJJAMCKhdsBgk99CmZmSd962g',
                },
        'data_range': {
                'TW': 'Copy of AC Conversion Lift in Taiwan ( Raw Data )',              
                },             
        }
...
unscaled_control_col_name = 'control'
control_col_name = 'scaled_control'
floodlight_col_name = 'conversion_segment'
country_col_name = 'country'
agg_floodlight_col_name = '1248666690 S2S - YouTube Music Premium - Global - Sign Up Complete - (from YT Music App)'
...
df = df[[country_col_name, floodlight_col_name, base_col_name, control_base_col_name,
         control_col_name, treatment_col_name, unscaled_control_col_name]]

df = df.drop_duplicates(keep='first')

agg_df = df.groupby(country_col_name)[country_col_name,
                   control_col_name, treatment_col_name,
                   unscaled_control_col_name].sum().reset_index()
...
df = df[[floodlight_col_name, country_col_name, base_col_name,
         control_base_col_name, treatment_col_name, unscaled_control_col_name,
         control_col_name]]
